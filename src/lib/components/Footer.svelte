<script>
	...
	let textStats = $derived(activeTab ? getTextStats(activeTab.content) : { words: 0, characters: 0, charactersNoSpaces: 0, readingTime: 0 });
	let formattedTime = $derived(formatDateTime(currentTime, dateTimeFormat));
</script>

<footer 
	class="border-t border-gray-300 bg-gray-50 px-4 py-2 text-xs text-gray-600 flex items-center justify-between flex-shrink-0"
...
>
	<div class="flex items-center gap-4">
		...
		{#if activeTab}
			...
			<span title="Character count (no spaces)">{textStats.charactersNoSpaces} chars (no spaces)</span>
